...
@interface RTSPServerConnection ()
{
    CFSocketRef _s;
    RTSPServer* _server;
...
- (RTSPServerConnection*) initWithSocket:(CFSocketNativeHandle) s Server:(RTSPServer*) server;
...
{
    RTSPServerConnection* conn = (__bridge RTSPServerConnection*)info;
    switch (callbackType)
    {
...
@implementation RTSPServerConnection

+ (RTSPServerConnection*) createWithSocket:(CFSocketNativeHandle) s server:(RTSPServer*) server
{
    RTSPServerConnection* conn = [RTSPServerConnection alloc];
...
- (RTSPServerConnection*) initWithSocket:(CFSocketNativeHandle)s Server:(RTSPServer *)server
{
    ...
    _s = CFSocketCreateWithNative(nil, s, kCFSocketDataCallBack, onSocket, &info); //服务端的socket，注意不要随便改
