...
static AVEncoder *_globalAVEncoder;

+ (AVEncoder *)sharedInstance
{
    @synchronized([AVEncoder class]){
        if (_globalAVEncoder == nil) {
            _globalAVEncoder = [[AVEncoder alloc]init];
            [_globalAVEncoder initForHeight:480 andWidth:720];
        }
    }
    return _globalAVEncoder;
}
...
- (void) onFileUpdate
{
    ...
    int cReady = (int)(s.st_size - [_inputFile offsetInFile]);//offsetInFile获取当前文件的偏移量
...
- (void) readAndDeliver:(uint32_t) cReady
{
    ...
    while (cReady > _lengthSize)
    {
        ...
        if (lenNALU > cReady)
        {
            ...
            [_inputFile seekToFileOffset:[_inputFile offsetInFile] - 4];//跳到指定文件的偏移量
            break;
        }
